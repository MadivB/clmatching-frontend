"""Targeted big-blob refinement scan (label-efficient DBSCAN improvement).
Base chain (eps 4.0, adopted B3) built once per event; then per variant
(T = min blob hits to refine, eps_f = fine eps) each blob label >= si with
>= T hits is re-clustered with DBSCAN(eps_f, min 3). The largest sub-cluster
AND all noise points keep the parent label (zero energy demotion); other
sub-clusters get fresh labels. Usage: python refine_worker.py <ptname> [...]"""
import copy, inspect, json, os, pickle, sys
from types import SimpleNamespace
import numpy as np
import torch
from sklearn.cluster import DBSCAN

REL = r'F:\CLMatching_v0.5.6_release'
SCR = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, REL)
from recluster import truth_groups, _file_load
import vertex_pinpoint as vp

OUT = os.path.join(SCR, 'refine')
os.makedirs(OUT, exist_ok=True)
tb = _file_load('tb_r', os.path.join(REL, 'clustering_v055',
                                     'global_track_clustering_toolbox_v11_2.py'))
sd = _file_load('sd_r', os.path.join(REL, 'stitch_directional.py'))
sp = _file_load('sp_r', os.path.join(REL, 'segment_split.py'))
er = _file_load('er_r', os.path.join(REL, 'endpoint_rejoin.py'))
params = json.load(open(os.path.join(REL, 'params_full.json')))
sig = set(inspect.signature(tb.build_global_labels_toolbox).parameters)
kw_base = {k: v for k, v in params.items() if k in sig}
kw_base['return_label_info'] = True
tb._match_segments_across_tpcs_toolbox = sd.make_matcher(
    SimpleNamespace(**params), tb._fit_line_metrics)

VARIANTS = [(500, 3.0), (500, 2.5), (300, 3.0), (300, 2.5), (150, 3.0)]

def metrics(labels, si, e, tg):
    okr = labels >= 0
    n_raw = len(np.unique(labels[okr]))
    ok = okr & (tg >= 0)
    ulab, li = np.unique(labels[ok], return_inverse=True)
    M = np.zeros((len(ulab), int(tg[ok].max()) + 1))
    np.add.at(M, (li, tg[ok]), e[ok])
    P = float(M.max(1).sum() / M.sum())
    C = float(M.max(0).sum() / M.sum())
    tr = ulab < si
    Pt = float(M[tr].max(1).sum() / max(M[tr].sum(), 1e-9))
    Pb = float(M[~tr].max(1).sum() / max(M[~tr].sum(), 1e-9))
    frn = float(M.sum() - M.max(1).sum())
    return P, C, Pt, Pb, frn, n_raw

rows = []
for ptname in sys.argv[1:]:
    blob = torch.load(os.path.join(REL, 'pt', ptname + '.pt'), weights_only=False)
    for ev_id, ev in blob['events'].items():
        with open(os.path.join(SCR, 'segs', f'{ptname}_ev{ev_id}.pkl'), 'rb') as f:
            seg0 = pickle.load(f)
        x = ev['x'].numpy().astype(np.float64)
        y = ev['y'].numpy().astype(np.float64)
        z = ev['z'].numpy().astype(np.float64)
        xyz = np.stack([x, y, z], 1)
        e = np.clip(ev['energy'].numpy().astype(np.float64), 0, None)
        tg = truth_groups(ev['truth_t0'].numpy().astype(np.float64))
        tpcv = ev['tpc'].numpy().astype(int)
        segments, _ = sp.split_segments(seg0, xyz, SimpleNamespace(**params))
        tb._build_tpc_segments_toolbox = lambda *a, **k: (segments, {})
        labels0, si, li0 = tb.build_global_labels_toolbox(
            x, y, z, ev['io_group'].numpy(), **kw_base)
        labels0 = np.asarray(labels0)
        li = copy.deepcopy(li0)
        pr_ = dict(params); pr_['er_bridge_occupancy'] = 0.5
        labels1, st = er.rejoin_endpoints(labels0, int(si), li, xyz, tpcv,
                                          SimpleNamespace(**pr_))
        labels_b, stv = vp.pinpoint_vertex_merge(
            labels_global=labels1, split_index=int(si), label_info=li,
            x=x, y=y, z=z, config=SimpleNamespace(**params), tpc=tpcv)
        labels_b = np.asarray(labels_b)
        si = int(si)
        P, C, Pt, Pb, frn, n_raw = metrics(labels_b, si, e, tg)
        rows.append(dict(pt=ptname, ev=int(ev_id), variant='base', P=P, C=C,
                         P_track=Pt, P_blob=Pb, foreign=frn, n_raw=n_raw,
                         n_refined=0))
        blob_labs = np.unique(labels_b[labels_b >= si])
        counts = {int(l): int((labels_b == l).sum()) for l in blob_labs}
        for T, eps_f in VARIANTS:
            lab_v = labels_b.copy()
            nxt = int(lab_v.max()) + 1
            n_ref = 0
            for l, c in counts.items():
                if c < T:
                    continue
                idx = np.flatnonzero(labels_b == l)
                sub = DBSCAN(eps=eps_f, min_samples=3).fit_predict(xyz[idx])
                u, cnt = np.unique(sub[sub >= 0], return_counts=True)
                if len(u) < 2:
                    continue
                n_ref += 1
                keep = u[cnt.argmax()]
                for uu in u:
                    if uu == keep:
                        continue
                    lab_v[idx[sub == uu]] = nxt
                    nxt += 1
                # noise points keep parent label (zero demotion)
            P, C, Pt, Pb, frn, n_raw = metrics(lab_v, si, e, tg)
            rows.append(dict(pt=ptname, ev=int(ev_id), variant=f'T{T}_e{eps_f}',
                             P=P, C=C, P_track=Pt, P_blob=Pb, foreign=frn,
                             n_raw=n_raw, n_refined=n_ref))
        print(ptname, ev_id, 'done', flush=True)
with open(os.path.join(OUT, f'rows_{sys.argv[1]}.pkl'), 'wb') as f:
    pickle.dump(rows, f)
print('worker done')
